...
function _medium1_data(FileAttachment){return(
FileAttachment("output.json").json()
)}

function _medium1(d3,medium1_data,drag,invalidation)
{
  // 指定圖表的尺寸。
  const width = 500;
  const height = 400;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(medium1_data);
...
function _medium2_data(FileAttachment){return(
FileAttachment("output.json").json()
)}

function _medium2(d3,medium2_data,drag,invalidation)
{
  // 指定圖表的尺寸。
  const width = 500;
  const height = 400;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(medium2_data);
  const links = root.links();
  const nodes = root.descendants();
...
function _medium3_data(FileAttachment){return(
FileAttachment("output.json").json()
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("medium1_data")).define("medium1_data", ["FileAttachment"], _medium1_data);
  main.variable(observer("medium1")).define("medium1", ["d3","medium1_data","drag","invalidation"], _medium1);
  main.variable(observer("medium2_data")).define("medium2_data", ["FileAttachment"], _medium2_data);
  main.variable(observer("medium2")).define("medium2", ["d3","medium2_data","drag","invalidation"], _medium2);
  main.variable(observer("medium3_data")).define("medium3_data", ["FileAttachment"], _medium3_data);
  main.variable(observer("medium3")).define("medium3", ["d3","medium3_data","drag","invalidation"], _medium3);
