...
function _simple2_data(FileAttachment){return(
FileAttachment("output.json").json()
)}

function _simple2(d3,simple2_data,drag,invalidation)
{
  // 指定圖表的尺寸。
  const width = 500;
  const height = 400;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(simple2_data);
  const links = root.links();
  const nodes = root.descendants();
...
function _simple3_data(FileAttachment){return(
FileAttachment("output.json").json()
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("simple2_data")).define("simple2_data", ["FileAttachment"], _simple2_data);
  main.variable(observer("simple2")).define("simple2", ["d3","simple2_data","drag","invalidation"], _simple2);
  main.variable(observer("simple3_data")).define("simple3_data", ["FileAttachment"], _simple3_data);
  main.variable(observer("chart")).define("chart", ["d3","simple3_data","drag","invalidation"], _chart);
